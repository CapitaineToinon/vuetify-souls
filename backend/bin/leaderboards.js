...
const formatLeaderboardsData = (game, category, runs, players) => {
    ...
    const formattedruns = runs.map(run => {
        ...
        // Player(s)
        const players_names = run.run.players.map(player => {
            /**
             * Name and flag
             */
            let name = "";
            ...
            if (player.hasOwnProperty("id")) {
                const fullPlayerObject = players.find(p => p.id === player.id);
                ...
                nameStyle = formatNameStyle(fullPlayerObject["name-style"]);
